...
class Enseignant_model extends MY_Model {
  ...
  return $row['emailTeacher'];
        ...
        public function MAILtoID($emailTeacher) {
   $query = $this->db->query("SELECT idTeacher FROM Enseignant WHERE emailTeacher ='$emailTeacher' ;");
  $row = $query->row_array();
  return $row['idTeacher'];     
    
}

    public function myNotes($idTeacher)
        {
          $query = $this->db->query("SELECT * FROM notedby WHERE idTeacher='$idTeacher' ;");
          return $query->result_array();
        }

                    public function getmyNotes($idTeacher)
        {
    $this->db->select('Etudiant.nomStudent, Etudiant.prenomStudent, Filiere.nomFiliere, Matiere.nomMatiere, notedby.annee, notedby.note_S1, notedby.note_S2, notedby.note_Oral');
$this->db->from('notedby');
$this->db->where(array('notedby.idTeacher' => $idTeacher));
$this->db->join('Etudiant', 'notedby.idStudent = Etudiant.idStudent');
$this->db->join('Matiere', 'notedby.idMatiere = Matiere.idMatiere');
$this->db->join('Filiere', 'notedby.idFiliere = Filiere.idFiliere');

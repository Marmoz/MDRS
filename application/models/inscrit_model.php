...
return $this->db->update('inscrit'); 
     ...
     public function give_AnneeScolaire()
      ...
      public function give_byID($idEtudiant)
        {
          $query = $this->db->query("SELECT * FROM inscrit WHERE idStudent='$idEtudiant' ;");
          return $query->result_array();
                }
    
                       public function give_byID2($idEtudiant)
        {
          $query = $this->db->query("SELECT * , inscrit.annee, Filiere.nomFiliere, Filiere.idFiliere
FROM Inscrit
LEFT JOIN Filiere ON inscrit.idFiliere = Filiere.idFiliere

WHERE inscrit.idStudent='$idEtudiant' ;

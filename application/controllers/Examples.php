...
/**
	 ...
	 * 1- Recup nom prenom email ROLE
	 2- ecrire MDP
	 3- creer user
	 4- ajouter user_id a Etudiant
	 */
public function dakhla($idEtudiant)
	{
		$data = array(
            'metaTitle' => 'Ajouter compte Etudiant | MadrassApp',
             'intelStudent' => $this->etudiant_model->givebyID($idEtudiant),
             'chkoun' => $idEtudiant	);
		...
		echo $this->load->view('template/adduser', '', TRUE);
...
public function creer_compteStudent()
	{
		// Customize this array for your user
		$user_data = [
			'username'   => NULL,
			'passwd'     => $this->input->post('lepassword'),
...
$data = array(
	...
	// ENVOI LIEN AVEC ETUDIANT DB
				$this->etudiant_model->dewwez($user_data['email'],$user_data['user_id']);
				if( $this->db->affected_rows() == 1 ){
									echo '<h1>AJOUT DB OK';

					}
					else{	echo '<h1>OOPS</h1> </p>';
}
			}
	...
	// TEACHER TEACHER T TEACHER EACHER TEACHER 

	public function dakhlaTeacher($idEnseignant)
	{
		$data = array(
            'metaTitle' => 'Ajouter compte Enseignant | MadrassApp',
             'intelTeacher' => $this->enseignant_model->givebyID($idEnseignant),
             'chkoun' => $idEnseignant	);

		if( $this->verify_min_level(9) )
		{
		echo $this->load->view('template/header', $data,  TRUE);
		echo $this->load->view('template/adduserteacher', '', TRUE);
		echo $this->load->view('template/footer', '', TRUE);
		}
...
public function creer_compteTeacher()
	{
		// Customize this array for your user
		$user_data = [
			'username'   => NULL,
...
$data = array(
		...
		if( $this->form_validation->run() )
		{
			...
			if( $this->db->affected_rows() == 1 ){
				...
				$this->enseignant_model->dewwezTeacher($user_data['email'],$user_data['user_id']);
				if( $this->db->affected_rows() == 1 ){
									echo '<h1>AJOUT DB OK';
	...
	{
		$data = array(
            'metaTitle' => 'Accueil | MadrassApp',	);

		$this->is_logged_in();
		
		echo $this->load->view('template/header', $data,  TRUE);
		echo $this->load->view('etudiant/portalstudent', '', TRUE);
		echo $this->load->view('template/footer', '', TRUE);
	...
	public function home()
	{
		...
		'listetudiants' => $this->etudiant_model->give_all(),
				'listefilieres' => $this->filiere_model->give_all(),
				'listefilieres' => $this->filiere_model->give_all(),
				'listematieres' => $this->matiere_model->give_all(),

		'listeenseignants' => $this->enseignant_model->give_all(),	);

		$this->is_logged_in();
		
		

if (isset($this->auth_role)) {


		if ($this->auth_role == "teacher") {
				echo $this->load->view('template/header', $data,  TRUE);


			$data['nozinfos'] = $this->enseignant_model->getinfobyUSERMAIL($this->auth_email);
			$data['noznotes'] = $this->etudiant_model->notebyUSERMAIL($this->auth_email);
			echo $this->load->view('enseignant/portalteacher', $data, TRUE);
						echo $this->load->view('template/footer', '', TRUE);

		}
		if ($this->auth_role == "admin") {

			echo $this->load->view('template/homeaccess', $data, TRUE);
		}
		if ($this->auth_role == "student") {
				echo $this->load->view('template/header', $data,  TRUE);

			$data['nozinfos'] = $this->etudiant_model->getinfobyUSERMAIL($this->auth_email);
			$data['noznotes'] = $this->etudiant_model->notebyUSERMAIL($this->auth_email);

			echo $this->load->view('etudiant/portalstudent', $data, TRUE);
			echo $this->load->view('template/footer', '', TRUE);
	...
	public function optional_login_test()
	{
		...
		else
		{
			...
			$page_content = '<p>Veuillez vous connecter. You are not logged in.</p>';
	...
	public function login()
	{
		...
		$data = array(
            'metaTitle' => 'Connectez vous | MadrassApp',
		);
